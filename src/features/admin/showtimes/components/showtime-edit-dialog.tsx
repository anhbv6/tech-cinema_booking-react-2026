...
} from "@/features/admin/showtimes/schemas/showtime.schema";
import type { Showtime } from "@/features/admin/showtimes/components/showtime-table";
...
export function ShowtimeEditDialog({
  ...
  const updateShowtimeMutation = useMutation({
    ...
    onError: (error: unknown) => {
        const message =
        (error as { response?: { data?: { message?: string } } })?.response?.data?.message || "Failed to update showtime";
        toast.error(message);
    },
    });
